...
function backupName(targetFile, now = new Date()) {
  const date = now.toISOString().slice(0, 10);
  return `${targetFile}.${date}.shared-archive-backup.json`;
}
...
if (require.main === module) {
  const [targetFile, sourceFile] = process.argv.slice(2).map((file) => path.resolve(file || ''));
...
module.exports = { mergeArchives, backupName, migrate };
