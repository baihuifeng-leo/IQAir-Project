...
EC-Workbench/
  ...
  public/
    ...
    reviews.js, preview3d.js, preview3d-scene.js, report.js, report-slides.js,
...
- `data/` — runtime data dir (`db.json`, `users.json`, uploads, reviews/reports/materialcheck data, `.session-secret`, `audit.log`, `platform-sessions/` [Taobao/Tmall controlled-browser login state], `detail-jobs/` [detail-long-image task records + result PNGs, 24h retention])
...
- `node_modules/` — npm deps (`sharp`, `playwright`, `fflate`; `jsdom` is dev-only) used solely by the detail-long-image feature; `npm ci` to install
- `*.log`
...
- Detail long image (详情长图, 素材中心's second sub-page): `taobao-session.js` owns a persistent, per-account Chromium context (`--no-sandbox --disable-dev-shm-usage`, launched from a forked child process — see `detail-worker.js` — not the main process) for logged-in Taobao/Tmall sessions; `taobao-detail-adapter.js` extracts image/text/table blocks from the detail DOM; `detail-image-resolver.js` downloads the candidate images (node:https, no npm HTTP client) with per-asset/per-task byte budgets; `detail-png-composer.js` streams them into one full-width PNG strip-by-strip via `sharp-operation-runner.js`, which runs each Sharp call in its own disposable child process specifically so an in-flight native operation can be SIGKILLed on cancel (a Worker thread in the same process can't be). `detail-task-store.js`/`platform-session-store.js` persist task/session state (scrubbing anything that looks like a credential before it ever touches disk); `detail-job-runner.js` (main process) owns a `DetailWorkerClient` IPC connection to the worker, turns its phase events into task-store transitions + SSE `detail-job` broadcasts; `detail-api.js` exposes `/api/platform-accounts*` and `/api/detail-jobs*`.

**Frontend** — no framework, no virtual DOM. `core.js` holds shared app state, the SSE client, debounced autosave (0.7s idle → `PUT /api/doc/:name`), the undo stack, and image compression before upload. Each top-level tab is its own file operating on shared state: `matrix.js` (price-band board), `compare.js` (competitor comparison), `reviews.js` (review sentiment board, uses ECharts), `preview3d.js` + `preview3d-scene.js` (3D scatter via Three.js — `preview3d.js` is the classic-script UI/data layer, `preview3d-scene.js` is the site's only ES module, bridged through `window.P3DScene` + the `p3dscene-ready` event), `report.js` (personal reports) + `report-slides.js` (its "自定义页" tab — a zero-dependency PPT-style slide editor: text/image/shape/symbol elements, drag/resize, a `ResizeObserver`-driven canvas that keeps a fixed 1280×720 coordinate space scaled to fit any container size, autosave, and `buildPrintPage()` for the PDF export path — every new element type must be rendered there too or it silently vanishes from exports), `materialcheck.js` (material/keyword compliance check tab), `platform-accounts.js` (Taobao/Tmall login session cards + the "素材中心" dropdown nav wiring), `detail-long-image.js` (submit a product URL, track phase/asset progress via polling + SSE, download the result), plus `admin.js` (audit log / backup-restore panel), `users.js` (user management), `settings.js` (per-user tab visibility). `styles.css` holds all design tokens and styling — no CSS-in-JS, no preprocessor.
