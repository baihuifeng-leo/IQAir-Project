...
**Frontend** — no framework, no virtual DOM. `core.js` holds shared app state, the SSE client, debounced autosave (0.7s idle → `PUT /api/doc/:name`), the undo stack, and image compression before upload. Each top-level tab is its own file operating on shared state: `matrix.js` (price-band board), `compare.js` (competitor comparison), `reviews.js` (review sentiment board, uses ECharts), `preview3d.js` + `preview3d-scene.js` (3D scatter via Three.js — `preview3d.js` is the classic-script UI/data layer, `preview3d-scene.js` is the site's only ES module, bridged through `window.P3DScene` + the `p3dscene-ready` event), `report.js` (personal reports) + `report-slides.js` (its "自定义页" tab — a zero-dependency PPT-style slide editor: text/image/shape/symbol elements, drag/resize, a `ResizeObserver`-driven canvas that keeps a fixed 1280×720 coordinate space scaled to fit any container size, autosave, and `buildPrintPage()` for the PDF export path — every new element type must be rendered there too or it silently vanishes from exports), `materialcheck.js` (material/keyword compliance check tab), plus `admin.js` (audit log / backup-restore panel), `users.js` (user management), `settings.js` (per-user tab visibility). `styles.css` holds all design tokens and styling — no CSS-in-JS, no preprocessor.
