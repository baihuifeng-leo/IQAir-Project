...
const POS_RULES = [
  ...
  /及时|热情|专业|迅速|(?<![不])快(?!递)/, /轻巧|轻便/, /干净|灵敏|精准/,
  /(降|降到|归)(0|零)|优秀|出色|完美/
];
...
const NEG_RULES = [
  ...
  /笨重|太重|占地方|太大了/,
  /(客服|售后|物流|快递)(态度)?(很|太|非常|比较|有点|挺|巨|超)?(差|慢|不理|敷衍)/,
  /刺鼻|塑料味|异味大/,
  /(?<![空气])质量(差|不行|堪忧)/,
  /难用|复杂|麻烦|不好用/
];
...
// 「没多久/没几天/没两下」是时间状语（没过多久），不是在否定后面的词——
// 不排除的话，"用了没多久就坏了" 会把「坏了」判成被否定 → 变成正向的"不坏了"，
// 差评混进好评（真实数据里出现过这个例子）。
const NEGATOR = /(?<![特识分差个类性])别|不(?!错|少|突兀)|没(?![什么]*(问题|噪音)|多久|几(天|次|下|分钟|小时))|无(?!线|声|噪)|未(?!来)|毫无|并非|算不上|谈不上/;
...
// 4. 贬旧褒新："才明白之前的净化器都是智商税" —— 负面词说的是"以前买的那台"，
//    是在夸当前买的这台，不是在骂它（真实数据里出过这个例子，被误判成了差评）
const OLD_PRODUCT = /(之前|以前|原来|原先|上一台|老款)(的|买的|那台|那个|用的)?.{0,10}(智商税|不好|没用|垃圾|坑|白买|不行|白花钱|踩雷)/;

// 「不仅/不但」是递进不是否定
const FAKE_NEGATOR = /不仅|不但|不光|无论|不管|没准|不知|不愧|不亏|不简单|别人|别的|别提/;

// 很多导出的评论用空格代替标点分句（"物流服务极差半个月才送到 客服很唐只会已读乱回 净化器挺好的"），
// 不拆的话一句里两个不相关话题会共用同一个极性，把差评的话题也染成好评。
// 中文本身不会在字之间留空格，所以「汉字 + 空格 + 汉字」基本等于一个分句边界；
// 英文/数字词组（"PM2.5"、"APP"）两侧不是汉字，不会被误拆。
const splitClauses = (text) =>
  normalize(text).split(/[，,。.！!？?；;、\n]+|(?<=[一-鿿])\s+(?=[一-鿿])/).map((s) => s.trim()).filter((s) => s.length >= 2);
...
function extract(text) {
  ...
  for (const clause of clauses) {
    ...
    if (score < 0) {
      ...
      if (OLD_PRODUCT.test(clause)) continue;
    }
