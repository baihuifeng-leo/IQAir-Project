...
assert.match(js, /const monday = archiveView \? new Date\(`\$\{archiveView\.weekStart\}T00:00:00`\) : startOfWeek\(date\);/, '历史报告文件名必须优先使用档案周');
assert.match(js, /if \(!archiveView\) monday\.setDate\(monday\.getDate\(\) - 7\);/, '当前报告文件名必须使用上一个完整自然周');
