...
## 会话：2026-08-03 / 08-04（AI 新闻生成工作流 + 周报档案版本化）

> 归并说明：这两条工作线当时用 `planning-with-files-zh` 的 `.planning/ai-news-generation/`、`.planning/report-archive/` 单独记录，没有写进本文件，导致 07-23 到 08-18 之间看起来断档。2026-08-19 清理过期 planning 文件时把时间线补回这里（那两个目录未被 git 跟踪，已删除；提炼后的决策见 Obsidian 知识库 `Decisions/AI新闻生成模型配置.md`、`Decisions/周报档案版本化.md`）。

### 工作线 A：AI 新闻生成与放映优化（2026-08-03，19 个阶段全部 complete）

把"选择两条新闻"从**基于首句的规则截取**（原 `briefFrom()`，根本没有模型调用）改成真正的 AI 工作流：读原文与封面信息后由配置的模型输出事实约束的中文摘要、关键要点和每条新闻的版式密度；普通报告展示完整摘要，放映模式只显示精炼讲稿。

模型接入与契约细节见知识库 `Decisions/AI新闻生成模型配置.md`（DeepSeek `deepseek-v4-flash`、`thinking: disabled`、固定 JSON 契约、不符合即拒绝发布、密钥只走服务端环境变量）。

主要迭代（几乎每一轮都是用户看了效果后反馈驱动的版式调整）：

- 放映稿改为正文配图优先 + 源站可跳转的双栏，内容是标题/结论/三条要点，而不是放大普通卡片
- 封面改前景图完整展示 + 同图模糊背景铺底，避免比例不一致时裁切主体；AI 摘要契约扩到 180–300 字
- 放映正文垂直居中；摘要按"发生了什么—为何重要—业务关注"组织；三条要点固定为事实/影响/关注，薄荷、蓝、暖色编码
- 修三项界面回归：普通新闻卡取消固定 400px 最小高度；只读态关闭按钮不再被全局删除按钮规则隐藏、支持 Esc/点遮罩关闭；主操作区保持单行。新增 `report-news-ui.test.js` 锁住这些结构性回归
- 放映页锁定单屏双栏，正文区/卡片/网格禁止纵向溢出；封面点击改工作台原图浮窗，源站跳转只留底部原文链接
- 按封面真实比例动态分类：横图缩短封面区扩大正文，竖图切左图右文，避免纵向素材占满卡片导致摘要消失
- 放映网格显式拉伸两张卡至同高；横竖图正文字号同步提升保持单屏可读
- 放映模式取消摘要三行截断（摘要作主叙事完整展示），后又按反馈恢复"事实／影响／关注"彩色要点，用摘要下方余高承载
- 自定义页工具栏加全屏编辑入口；报告页签支持固定页与自定义页混合拖拽排序，持久化后编辑和放映共用顺序 — commit `9d60fbb`，已推 `main` 并部署生产
- 微盟页新增按周汇总的公众号/小程序/APP 浏览量趋势折线图，保存刷新、主题切换、放映切换均自动重绘或缩放 — commit `7118cff`
- 趋势图对齐第一页折线视觉（同一套图例/坐标轴/提示浮层/薄荷面积填充）；放映页压缩为单行八项指标卡+趋势图+渠道构成+点评 — commit `51c0b2f`
- 按反馈把趋势图移到标题下方，放映恢复浏览量/访客数两张重点大卡，其余指标转次级网格 — commit `ed87719`
- **修一个真 bug**：自定义页空白画布点击导致全屏退出 — 根因是空白点击触发重渲染，旧全屏根 `.rs-shell` 被替换后浏览器自动退出全屏；改成用稳定的自定义页宿主容器作全屏根 — commit `9cb2bea`，已推 `main` 并部署生产，8090 健康检查 + 代码一致性 + 新闻文件哈希均通过

### 工作线 B：个人报告周报档案版本化（2026-08-03/04，4 阶段 + 后续迭代，全部 complete）

让每个账户可归档每周个人报告，回溯/放映/导出指定周的正式版本，并能在保留原始版本的前提下编辑生成修订版。核心决策见知识库 `Decisions/周报档案版本化.md`。

- 阶段 1-3：归档模型与接口（周/版本/正式版本/快照存储，归档列表-读取-创建修订-保存-设为正式版接口 + 测试）；标题栏"周报档案"入口和档案抽屉；放映与 PDF 导出改用当前选中档案版本
- 阶段 4：验证与部署 — commit `02190e7` 已上生产
- 后续迭代（08-04）：归档后清空实时自定义页与排序、前端回到第 1 页；重命名弹窗操作区比例与对齐；自定义页提供可见的删除按钮；后又把重命名/删除从页签移到画布编辑工具条最右侧，由报告层统一处理弹窗与删除确认
- 验证：`report-store.test.js`、`report-archive-ui.test.js`、`report-page-order-ui.test.js`、`report-slides-editing.test.js`、语法检查、`git diff --check` 全部通过；界面检测无新增告警

⚠️ 这条工作线埋下的隐患在 2026-08-18 集中爆发（见下方三个会话）：归档相关的"周与周互相污染"共发生两次。
...
## 会话：2026-08-18（续）：用户纠正——AI 新闻本来就该手动挪到 Q3W07，不是留在本周

### 状态：complete

我判断错了一次：上一节把"微软退出中国/DeepSeek涨价"当成本周（08-17）的内容重新生成完就结束了，用户明确要求把它挪进 Q3W07（`2026-08-10`）归档里，不是留在本周。核对另外两个已归档周（`2026-07-27`、`2026-08-03`）后发现一个之前没注意到的规律：三周的 `snapshot.news.publishedAt` **全部**比归档周晚 1～8 天（`07-27` 周的新闻是 `08-03` 生成的，`08-03` 周的新闻是 `08-10` 生成的）——**这是用户一贯的真实工作流程：新的一周开始后，回头给上一周的报告挑 2 条当周最重要的 AI 新闻，不是必须在那一周内当天生成**。我之前"按生成时间戳判断归属周"的推理本身没错，但漏了这条已经稳定重复了两次的使用模式，误把"当周生成"当成了唯一正确的判断标准。

同时翻到 `docs/superpowers/specs/2026-08-11-archive-window-and-news-binding-design.md`——这份 8-11 的设计文档记录了**几乎一模一样的历史事故**：`2026-07-27` 档案当时也被错误地存进了 `2026-08-03` 的新闻（原因是旧版 `archiveCreate` 在目标周没新闻时会 `|| summary.news` 回退取"本周"新闻），修复方式是去掉这个回退、目标周没新闻就存 `null`，绝不自动拿别的周顶替。这跟这次 `clearLiveNews` 误删本周新闻是**同一类"周与周之间互相污染"的问题，这是第二次发生**，只是诱因不同（一个是错误的自动回退，一个是清空范围太大）。这份文档也确认了"档案数据绝不能自动跨周替代"是这个功能一贯坚持的设计原则——这次挪动是**用户手动指定**的编辑操作，不违反这条原则，跟历史那次"系统自动张冠李戴"的 bug是两回事。

### 执行的操作
用一次性 Python 脚本（跟之前恢复 Q3W07 的手法一致）：
1. 备份 `reports/u_fb623958.json` 和 `report-news/u_fb623958.json`（改前各存一份时间戳备份）。
2. 把本周（`weekStart=2026-08-17`）刚生成的新闻整块搬进 `2026-08-10` 归档版本的 `snapshot.news`，并把里面的 `weekStart` 字段改成 `2026-08-10`（保持跟另外两周归档的字段语义一致）。
3. 从实时新闻库的 `weeks`/`drafts` 里删掉 `2026-08-17` 这个 key（搬走了就不该再留在"本周"工作区里）。
4. `node -e "JSON.parse(...)"` 校验两个文件写回后仍是合法 JSON。

验证：三个归档周现在分别是——`07-27`→京东AI头盔/黄仁勋开源新闻，`08-03`→DeepSeek入股宇树/苹果长鑫存储新闻，`08-10`→DeepSeek涨价/微软退出中国新闻，`news.weekStart` 字段跟各自归档周完全对应；本周（`08-17`）实时新闻库清空，等用户重新为当前周挑新闻。

### 五问重启检查
| 问题 | 答案 |
|------|------|
| 我在哪里？ | Q3W07 归档现在存的是用户真正要的新闻内容（微软退出中国/DeepSeek涨价）；本周新闻工作区已清空等待重新生成 |
| 我要去哪里？ | 等用户确认三周归档现在都显示正确；可选：如果用户想要，可以调研"在归档窗口里直接生成/绑定该周新闻"这条设计好的路径这次为什么没被走通（本周是从主界面"本周新闻"面板生成后才归档的，不是在归档窗口里生成的）——但用户没要求排查这个，先不主动做 |
| 目标是什么？ | 让生产数据准确反映用户的真实编辑意图，而不是我用时间戳推断出的"看起来自洽"但其实错的结论 |
| 我学到了什么？ | 涉及用户自己的历史编辑决策/业务内容判断时，即使有看起来完整自洽的时间线证据，也应该优先信任用户的直接纠正而不是继续用同一套推理去说服他们——尤其是当同类模式已经在其它样本（这次是另外两个归档周）里重复出现、而我一开始没有把所有样本放在一起比较的时候。改数据前一定要把"相似案例"都拉出来比一遍，不能只看眼前这一个 |
| 我做了什么？ | 见上方"执行的操作"；只改了生产 `reports/u_fb623958.json` 和 `report-news/u_fb623958.json` 两个数据文件（均已备份），没有改任何代码 |

---

## 会话：2026-08-18（续）：AI 新闻生成加"目标期数"选择器（用户主动提议）

### 状态：complete

用户看完前面两次事故（历史上的 07-27 自动回退 bug + 这次的 clearLiveNews 误删）之后主动提议："能不能给周报归档加个序号，用序号定义动作对象，会不会更合理？"。跟用户讨论后达成一致：根子不是"日期 vs 序号"这个身份标识方案的问题（序号一样会默认取"当前"，只是换个变量名重蹈覆辙），而是生成新闻的入口从来没让用户明确选过"这条新闻是给哪一期用的"——永远隐式等于"今天所在的这一周"。用户认可后，改成加一个显式的目标期数选择器。

### 实现
- `public/index.html`：新闻编辑弹窗里加了 `<select id="rpt-news-target">`，配一行说明文字。
- `public/report.js`：
  - 新状态 `newsTarget`（`null`＝本周，非空＝目标归档周的 weekStart）。
  - `renderNewsTargetSelect()`：选项＝"本周（日期范围）" + 所有已归档周（`FY26 QxWxx · 日期范围（已归档）`，复用已有的 `archivePeriodLabel`/`archiveLabel`，跟用户自己说话时用的格式完全一致）。
  - `openNewsPicker()` 打开时先 `newsTarget = null` 复位、`await loadArchives()` 拉最新归档列表（原来只有点开归档抽屉才会加载，新闻弹窗自己也得保证有数据）；`closeNewsPicker()` 关闭时同样复位——不管是生成完关闭还是点叉/点遮罩关闭，都不会有残留的目标选择漏到下一次。
  - `importNewsUrl()`/`saveNewsDraft()` 改成用 `newsTarget || news.weekStart`，不再写死本周。
  - 新增 `bindNewsToArchive(targetWeek, generatedNews)`：目标是已归档周时，生成成功后直接调用现有的 `archive/save` 接口，只替换 `snapshot.news`、`report` 原样保留（先 `GET archive` 拿当前 report，再整体 PUT 回去）——不走"生成进实时工作区、回头再手动挪"这条已经出过两次事故的路径。
  - 绑定前有 `confirm()` 二次确认（会覆盖该期已有新闻），避免手滑。
- 新增静态契约测试 `report-news-target-ui.test.js`，锁住上述关键行为（选择器存在、选项包含归档周、打开/关闭必须复位、生成走对应目标、已归档目标必须走绑定而不是停在实时工作区、绑定必须保留原 report 内容、覆盖前必须确认）。

### 验证
- `node --test *.test.js`：17 个测试文件全部通过。
- 真机 Playwright 验证（headless Chromium，本地 scratch 数据，非生产）：登录 → 报告管理 → 个人报告 → 切到"AI 新闻速览"页 → 打开"选择两条新闻" → 选择器正确显示"本周"+种下的一个归档周（`FY26 Q3W06 · 2026.08.03–08.09（已归档）`）→ 切换选项后弹窗标题同步变成"选择 FY26 Q3W06 两条 AI 新闻"→ 关闭再重新打开，标题和选中值都正确复位回"本周"→ 全程 0 个 console error。没有跑通"实际生成并绑定"这一步的真机验证（需要真实 AI 凭据和候选新闻数据，scratch 环境没配），这部分靠的是复用已经在生产验证过的 `archive`/`archive/save` 接口，加上上面的静态契约测试。

### 部署
已提交 `main`（commit `f01caab`）、部署到生产（8090，服务已重启、健康检查通过）、9090 测试环境也已同步到同一 commit 并重启。

### 五问重启检查
| 问题 | 答案 |
|------|------|
| 我在哪里？ | 目标期数选择器已上线生产；本周和 Q3W07 的新闻内容都已经是正确的（上一节手动纠正过）；这个新功能面向的是"以后"不再需要这种手动纠正 |
| 我要去哪里？ | 等用户在生产上实际用一次"回头给某一期补录新闻"，确认体验符合预期；如果用户后续还想给 `reports`/`report-news` 加更强的一致性保护（比如归档创建时校验 news.weekStart 必须等于目标周），可以再聊 |
| 目标是什么？ | 把这次讨论出的产品方案做成真实功能，而不是停留在口头共识，且要经得住这个项目一贯的真机验证标准 |
| 我学到了什么？ | 用户提出一个具体技术方案（"加序号"）时，价值往往在于背后想解决的真实场景，不一定要照单全收字面提议——跟用户对齐"要解决什么问题"之后一起换成代价更小、更贴合现有数据模型的方案，用户是认可的；`archives` 列表原来只在打开归档抽屉时才加载，新写的功能如果依赖同一份数据，不能想当然认为它已经加载好了，要主动确认/触发 |
